...
barchart<-function(x,df=titanic){
  ggplot(df)+geom_bar(aes_string(x=x),fill="light blue") 
}

h_barchart<-function(x,df=titanic){
...
lineplot<-function(x,y,df=titanic){
  ggplot(df)+geom_line(aes_string(x=x,y=y))
}

scatterplot<-function(x,y,df=titanic){
  ggplot(df)+geom_point(aes_string(x=x,y=y))
}

linear_scatterplot<-function(x,y,df=titanic){
  ggplot(df,aes_string(x=x,y=y))+geom_point()+stat_smooth(method="lm")
}

smooth_scatterplot<-function(x,y,df=titanic){
  ggplot(df,aes_string(x=x,y=y))+geom_point()+geom_smooth()
}

smooth_scatterplot_boolean<-function(x,y,df=titanic){
...
#facet_bar: can't make it work inside a function
  #ggplot(titanic)+geom_bar(aes(x=x),position="dodge",fill="light blue")+facet_wrap(~y,scales="free_y")+theme(axis.text.x=element_text(angle=45,hjust=1))
